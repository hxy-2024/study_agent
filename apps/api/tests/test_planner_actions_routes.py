...
async def test_start_review_uses_authorized_context(monkeypatch) -> None:
    tenant_id = uuid.uuid4()
    user_id = uuid.uuid4()
    action_id = uuid.uuid4()
    study_space_id = uuid.uuid4()
    ...
    async def fake_start_review_for_planner_action(**kwargs):
        captured_kwargs.update(kwargs)
        return SimpleNamespace(
            action=SimpleNamespace(
                ...
                title="Review",
                rationale="Reason",
                payload={"execution": {"review_session_id": str(session_id)}},
                created_at=None,
                updated_at=None,
            ),
    ...
    monkeypatch.setattr(routes_planner_actions, "start_review_for_planner_action", fake_start_review_for_planner_action)
    app.dependency_overrides[get_db_session] = fake_get_db_session
    app.dependency_overrides[get_authorized_user_context] = fake_get_authorized_user_context
    try:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            response = await client.post(f"/api/v1/planner-actions/{action_id}/start-review")
    finally:
        app.dependency_overrides.clear()
